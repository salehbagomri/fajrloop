...
exports.escalateUnconfirmedWakes = exports.cleanupOldDailyRecords = exports.onNewChatMessage = exports.onWakeConfirmed = exports.onChallengeComplete = exports.onEmergencyPanic = void 0;
 ...
 */
exports.onEmergencyPanic = functions.database.ref('/dailyRecords/{halqaId}/{date}/{userId}')
    ...
    const tokenSnaps = await Promise.all(memberIds.map(id => admin.database().ref(`/users/${id}/fcmToken`).once('value')));
 ...
 */
exports.onChallengeComplete = functions.database.ref('/dailyRecords/{halqaId}/{date}/{userId}')
    .onWrite(async (change, context) => {
    const before = change.before.val();
    const after = change.after.val();
 ...
 */
exports.onWakeConfirmed = functions.database.ref('/dailyRecords/{halqaId}/{date}/{userId}')
    .onWrite(async (change, context) => {
    const before = change.before.val();
    const after = change.after.val();
 ...
 */
exports.onNewChatMessage = functions.database.ref('/chatMessages/{halqaId}/{messageId}')
 ...
 */
exports.cleanupOldDailyRecords = functions.pubsub
        ...
        halqasSnap.forEach((halqaSnap) => {
            ...
            halqaSnap.forEach((dateSnap) => {
                ...
                if (dateStr < cutoffDateStr) {
                    deletePromises.push(db.ref(`/dailyRecords/${halqaId}/${dateStr}`).remove());
                    deletedCount++;
                }
            });
        });
 ...
 */
exports.escalateUnconfirmedWakes = functions.pubsub
    ...
    recordsSnap.forEach((halqaSnap) => {
        ...
        todaySnap.forEach((userSnap) => {
            ...
            const challengeDoneAt = userSnap.child('challengeDoneAtMillis').val();
            ...
            if (status === 'challenge_done' && challengeDoneAt && challengeDoneAt < tenMinutesAgo) {
                escalatePromises.push((async () => {
                    ...
                    const chain = halqaSnap2.child('chain').val() || [];
                    ...
                    if (!nextMemberId || nextMemberId === userId)
                        return;
                    // تسجيل panic تلقائي
